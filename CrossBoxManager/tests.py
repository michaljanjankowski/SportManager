...
from .models import Athletes, People, SportClub, Table


class ClubDetailsTests(TestCase):
    def setUp(self):
        self.club = SportClub.objects.create(
            club_name="Test club", address="Test address", nip="123",
            bank_account="123", founder="Founder", description="Description",
        )
    ...
    def test_user_with_club_profile_can_open_club(self):
        People.objects.create(
            user=self.user, sport_club=self.club, athlethe=Athletes.objects.create(),
        )
        self.client.force_login(self.user)
        response = self.client.get(self.url)
        self.assertContains(response, self.club.club_name)
    ...
    def test_missing_or_invalid_club_returns_404(self):
        ...
        self.assertEqual(self.client.get(reverse(
            'sport_club_details_by_id', args=[self.club.pk + 1],
        )).status_code, 404)
...
class PersonalMessagesTests(TestCase):
    def setUp(self):
        self.club = SportClub.objects.create(
            club_name="Inbox club", address="Address", nip="123",
            bank_account="123", founder="Founder", description="Description",
        )
    ...
    def test_account_without_profile_has_no_inbox(self):
        ...
        Table.objects.create(
            sport_club=self.club, to_who=None, message="Unassigned private message",
            date_posted=timezone.now(), typeOfmessage="InterStaff",
        )
        self.client.force_login(self.user)
        response = self.client.get(self.url)
    ...
    def test_inbox_only_shows_own_messages_in_requested_club(self):
        ...
        other_club = SportClub.objects.create(
            club_name="Other club", address="Address", nip="456",
            bank_account="456", founder="Founder", description="Description",
        )
        for club, recipient, content in [
            ...
            Table.objects.create(
                sport_club=club, to_who=recipient, message=content,
                date_posted=timezone.now(), typeOfmessage="InterStaff",
            )
        self.client.force_login(self.user)
        response = self.client.get(self.url)
    ...
    def test_empty_inbox(self):
        ...
        self.client.force_login(self.user)
        self.assertContains(self.client.get(self.url), "You have no messages in this club")

    def test_missing_club_returns_404(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('msg_show_to_person', args=[self.club.pk + 1]))
...
class MessageSendingTests(TestCase):
    def setUp(self):
        self.club = SportClub.objects.create(
            club_name="Messaging", address="Address", nip="123",
            bank_account="123", founder="Founder", description="Description",
        )
        self.admin = User.objects.create_superuser(
            username="sender_admin", email="admin@example.com", password="password",
        )
    ...
    def test_recipient_must_belong_to_selected_club(self):
        self.client.force_login(self.admin)
        self.recipient.sport_club = None
        self.recipient.save()
